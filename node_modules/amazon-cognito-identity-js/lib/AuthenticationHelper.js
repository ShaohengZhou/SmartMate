...
var randomBytes = function randomBytes(nBytes) {
  return _buffer.Buffer.from(CryptoJS.lib.WordArray.random(nBytes).toString(), 'hex');
};
...
var AuthenticationHelper = function () {
  ...
  AuthenticationHelper.prototype.hash = function hash(buf) {
    var str = buf instanceof _buffer.Buffer ? CryptoJS.lib.WordArray.create(buf) : buf;
  ...
  AuthenticationHelper.prototype.computehkdf = function computehkdf(ikm, salt) {
    var infoBitsWordArray = CryptoJS.lib.WordArray.create(_buffer.Buffer.concat([this.infoBits, _buffer.Buffer.from(String.fromCharCode(1), 'utf8')]));
    var ikmWordArray = ikm instanceof _buffer.Buffer ? CryptoJS.lib.WordArray.create(ikm) : ikm;
    var saltWordArray = salt instanceof _buffer.Buffer ? CryptoJS.lib.WordArray.create(salt) : salt;
